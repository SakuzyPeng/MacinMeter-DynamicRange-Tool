...
void ResultsDialog::show_results(const std::vector<DrAnalysisResult>& results, 
    ...
    std::string title = (results.size() == 1) ? 
        "MacinMeter DR Analysis Result (foobar2000 decoded)" : 
        PFC_string_formatter() << "MacinMeter DR Analysis Results (" << results.size() << " tracks, foobar2000 decoded)";
...
std::string ResultsDialog::generate_results_text(const std::vector<DrAnalysisResult>& results,
    ...
    if (batch_mode) {
        ...
        for (size_t i = 0; i < results.size() && i < track_infos.size(); ++i) {
            ...
            // 安全格式化官方DR值
            std::string dr_str = (std::isfinite(result.official_dr_value) && result.official_dr_value > 0)
                                 ? "DR" + std::to_string((int)std::round(result.official_dr_value))
                                 : "DR--";

            oss << track.file_name << "\t"
                << dr_str << "\t"
                << format_db_value(result.peak_db) << "\t"
                << format_db_value(result.rms_db) << "\t"
                << result.sample_rate << "Hz\t"
                << result.channels << "\t"
                << std::fixed << std::setprecision(1) << result.duration_seconds << "s\n";
        }
    } else {
...
std::string ResultsDialog::format_single_result(const DrAnalysisResult& result, const AudioData& track) {
    std::ostringstream oss;
    ...
    if (result.channels == 1) {
        ...
        oss << "                 Left              Right\n\n";
        oss << "Peak Value:     " << format_db_value(result.peak_db_per_channel[0]) << " dB         "
            << format_db_value(result.peak_db_per_channel[1]) << " dB   \n";
        oss << "Avg RMS:       " << format_db_value(result.rms_db_per_channel[0]) << " dB        "
    ...
    if (std::isfinite(result.precise_dr_value) && result.precise_dr_value > 0) {
        oss << "Precise DR Value: " << std::fixed << std::setprecision(2) << result.precise_dr_value << " dB\n\n";
    } else {
...
void ResultsDialog::show_results_dialog(const std::string& title, const std::string& content,
    ...
    std::string summary = track_infos.size() == 1 ?
        ...
        PFC_string_formatter() << "Batch DR analysis of " << track_infos.size() << " tracks completed! Check Console for detailed results.";
...
bool ResultsDialog::save_results_to_file(const std::string& content, const std::vector<AudioData>& track_infos) {
    if (track_infos.empty()) {
        popup_message::g_complain("MacinMeter DR Plugin", "No track information available for saving");
        return false;
    }
    ...
    try {
        ...
        if (!file) {
            popup_message::g_complain("MacinMeter DR Plugin", 
                PFC_string_formatter() << "Cannot create file: " << default_path.c_str());
            return false;
        }
