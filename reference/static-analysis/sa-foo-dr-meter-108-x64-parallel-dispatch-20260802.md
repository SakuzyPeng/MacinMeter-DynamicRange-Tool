...
本记录的直接用途是补齐一处此前未登记的结构关系：既有隔离 core 记录直接调用
analyzer 三入口并绕过调度器，完整组件则包含一个能够在三入口之上 fork-join 的
外层调度器；两条既有记录的窄字段对照精确匹配。既有 foobar observation 没有
动态记录调度分支或实际线程数，因此本记录不把那次 report 表述为已观测的多线程
运行。
...
结合本记录：前者来自不经 `0xdf30` 的单线程路径；后者来自完整 foobar 宿主路径，
但既有 observation 没有记录它是否进入 `0xdf30` 的 `n > 1` 分支。两者在这些公开
字段上逐位一致，证明的是直接单线程 core 与完整宿主路径的有界字段一致；该结果
与并行调度保持结果不变相容，但不单独证明那次 report 实际使用多线程。
...
- 并行的工作分割粒度、负载均衡策略或实际线程数（`_Thrd_hardware_concurrency`
  的返回值与工作量需求均未在本记录中动态观测）；也不建立既有 foobar report
  运行实际进入 `n > 1` 分支；
