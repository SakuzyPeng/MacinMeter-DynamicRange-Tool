...
try {
    ...
    # 2. 启动与监控（完全串行模式：关闭所有并行）
    # --------------------------------------------------
    Write-Host "正在启动 $($executableFile.Name) (完全串行模式)..."

    # (优化) 启动高精度计时器
    $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()

    # 启动程序，传入完全串行参数（不传入路径，让程序自己扫描当前目录）
    $process = Start-Process -FilePath $executableFile.FullName `
                             -ArgumentList @("--serial", "--no-parallel-files") `
    ...
    Write-Host "运行模式: 完全串行 (--serial --no-parallel-files)"
    Write-Host "扫描目录: 程序所在的可执行文件目录"
    Write-Host ""
