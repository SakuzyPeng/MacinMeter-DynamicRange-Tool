...
class PerformanceBaselineTests(unittest.TestCase):
    ...
    def test_overlap_handoff_gate_binds_requested_and_applied_depth(self) -> None:
        ...
        frames = corpus.SERIAL_ROUTE_SWEEP_FRAMES
        case = baseline.BenchmarkCase(
            "overlap-handoff/wave-s16-240s-d4",
            "application",
        ...
        too_wide = dataclasses.replace(
            case,
            case_id="overlap-handoff/wave-s16-240s-d4096",
            arguments=case.arguments[:4] + ("4096",),
        )
        sample["caseId"] = too_wide.case_id
        with self.assertRaises(baseline.BaselineError):
            baseline.validate_corpus_work([sample], [too_wide], root, manifest)

    def test_overlap_handoff_cases_are_explicit_and_cover_the_depth_grid(self) -> None:
        root = Path("/corpus")
        ...
        self.assertEqual(len(cases), 2 * len(baseline.OVERLAP_HANDOFF_DEPTHS))
        for track in ("wave-s16-240s", "aiff-s16-240s"):
            for depth in baseline.OVERLAP_HANDOFF_DEPTHS:
                self.assertIn(f"overlap-handoff/{track}-d{depth}", case_ids)
    ...
    def test_application_worker_gate_binds_packet_engine_subdivision(self) -> None:
        for suffix, container, codec, engine, decoder_workers, hasher_workers in (
            ...
            with self.subTest(suffix=suffix, missing="appliedOverlapChannelDepth"):
                missing = dict(details)
                del missing["appliedOverlapChannelDepth"]
                with self.assertRaises(baseline.BaselineError):
                    baseline.assert_application_allocation(
                        case,
                        missing,
