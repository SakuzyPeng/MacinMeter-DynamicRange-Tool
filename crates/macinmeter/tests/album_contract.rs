...
#[test]
fn official_album_uses_public_f32_track_values_instead_of_display_integers() {
    let tracks = [
    ...
    assert_eq!(result.official_dr_db.get().to_bits(), 0x412d_2c5f);
    assert_eq!(result.rounded_official_dr, 11);
    assert_eq!(
        result.duration_weighted_dr_db.unwrap().get().to_bits(),
        0x412d_2c5f
    );
    assert_eq!(result.rounded_duration_weighted_dr, Some(11));
    assert_eq!(result.effective_dr_db, result.official_dr_db);
...
#[test]
fn a_numeric_silent_track_is_included_in_the_official_album_mean() {
    let tracks = [
    ...
    assert_eq!(result.official_dr_db.get().to_bits(), 0x40a7_d70a);
    assert_eq!(result.rounded_official_dr, 5);
    assert_eq!(result.track_count, 2);
}

#[test]
fn duration_weighting_uses_exact_decoded_frame_durations() {
    let tracks = [
        track(10.0_f32.to_bits(), 8_000, 8_000),
        track(20.0_f32.to_bits(), 24_000, 8_000),
    ];

    let weighted = AlbumAggregator::aggregate(&tracks, AlbumWeighting::DurationWeighted).unwrap();
    assert_eq!(weighted.official_dr_db.get().to_bits(), 15.0_f32.to_bits());
    assert_eq!(
        weighted.duration_weighted_dr_db.unwrap().get().to_bits(),
        17.5_f32.to_bits()
    );
    assert_eq!(weighted.effective_dr_db.get().to_bits(), 17.5_f32.to_bits());
    assert_eq!(weighted.rounded_effective_dr, 18);
    assert_eq!(weighted.total_duration_seconds.get(), 4.0);
...
#[test]
fn zero_total_duration_omits_weighted_value_and_falls_back_to_official() {
    let tracks = [
    ...
    assert_eq!(result.official_dr_db.get(), 15.0);
    assert_eq!(result.rounded_official_dr, 15);
