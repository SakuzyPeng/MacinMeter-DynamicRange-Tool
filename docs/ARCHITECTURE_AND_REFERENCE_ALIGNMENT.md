...
>
> post-M6 并发方向：
> [ADR-0014：确定性有界并行与 packet 解码优先](adr/0014-deterministic-decode-analysis-pipeline.md)（Accepted / In progress；packet 级已为 ALAC 与适配资源几何的 FLAC route 默认启用；decode-analysis overlap 与文件级 lane 均已毕业并默认启用，窗口级未实施）
