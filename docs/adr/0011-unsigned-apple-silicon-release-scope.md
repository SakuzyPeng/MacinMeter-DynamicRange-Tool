...
- 将来加入 Intel/universal 或其他平台时，需要新的证据与独立决策。

本决策扩展 ADR-0010 的“CI 不保留制品”边界；ADR-0010 的 PR/main 正确性门禁、
clean staging、只读权限及不自动发布约束继续有效。

## 补充（2026-08-09）：不签名是长期决定，不是待办

上文「当前没有 Developer ID 签名与 notarization 需求」写的是 0.2.0 的范围裁剪，读起来
像时机问题。**它不是。** 对个人维护者而言不签名有一条不随时间改变的理由，记录在此，
以免后来人把它当成待修复的缺口。

### 理由：个人签名等于公开法定姓名

个人 Apple Developer Program 签发的证书，其 Subject 固定为
`Developer ID Application: <法定姓名> (TEAMID)`。**该 CN 嵌入每一个签名产物**，任何
拿到二进制的人都能用 `codesign -dvvv` 读出来。这是证书结构，不是可配置项。

也不存在“匿名签”的折中：要让证书显示机构名，必须以 organization 身份注册，而那需要
D-U-N-S 与法律实体登记——**暴露面更大，不是更小**。

Windows 侧不假定更宽松。CA/B 论坛收紧后代码签名密钥须置于硬件令牌或云 HSM，而个人
OV 证书的 Subject 同样是本人姓名。具体档位与条款需在实际决定分发时另行核实，但本 ADR
不以“Windows 可以匿名签”为前提。

### 因此首选分发路径是源码构建，而不是“未签名的预编译包”

`com.apple.quarantine` 由**下载它的那个程序**写入（浏览器等），`curl`、`git` 与
Homebrew 的源码构建路径都不写入。因此本地编译出的二进制没有该属性，Gatekeeper 不会
对它求值——**签名对这条路径本就没有意义**。这是路径性质，不是绕过手段。

据此的分发形态：

| 制品 | 路径 |
| --- | --- |
| CLI | 个人 tap 的源码构建 formula（`depends_on "rust" => :build`），签名问题不出现 |
| GUI | cask 分发预编译 `.app` 默认仍被 quarantine，因此要么只文档化从源码 `tauri build`，要么维持未签名 DMG 并持续给出明确的 `xattr` 说明 |

### 这条决定不改变什么

- 0.3.0 现有文案已如实写明未签名、未公证，无需修改；
- `stage-release.py` 的 unsigned candidate 契约、`SHA256SUMS` 与制品 smoke 全部继续有效；
- 校验和建立字节同一性，不是签名——该区分本来就写在发行文档里，不因本节改变；
- 若将来出于其他考虑（例如以机构身份分发）重新评估签名，那是一次新的独立决策，需要
  推翻本节记录的理由，而不是补齐一个待办。
