...
这不是一个待办项，而是可预见未来内的既定立场。阻碍不是费用或流程，而是隐私：只要
维护者仍是个人，签名就等于把法定姓名随每份公开制品一起发布。用户文案必须明示这一点，
不得写成“尚未签名”这类暗示即将改变的措辞——那会让用户误以为等下一版就能拿到已签名的
构建。若将来该立场改变，需要新的 ADR 说明是什么条件变了。
