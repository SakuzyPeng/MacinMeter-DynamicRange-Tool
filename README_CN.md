...
- `--timing` 会额外给出解码与分析各自的占用时间。**根据实际解码路径和资源计划，
  两者可能串行也可能重叠；这些数字还省略了其他工作，因此不是对总耗时的划分**：
  不可相加、不可当作总耗时的百分比、也不可反演成串行占比。每个计时间隔都要在
  开始和结束各读一次时钟；普通数据 block 的解码与分析各读两次，共四次。因此它
  需要显式开启；不加该 flag 时，分析器不会进行这些按 block 的时钟读取。两种方式
  的分析结果完全相同。
