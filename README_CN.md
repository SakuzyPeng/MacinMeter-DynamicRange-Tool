...
- `--timing` 会为每个角色额外输出一行：
  ...
  一个角色的 span 从它第一次活动持续到最后一次，`active + other` 恰好还原该
  span。**这些数字能划分的只有这一个 span。** `other` 是该角色窗口内的其余部分：
  block 交接、进度事件，以及在两者未重叠的路径上，另一个角色的工作。**两个 span
  之间可能重叠，重叠多少无法从这些数字还原** —— 两组区间各自的总长不决定它们的
  交集。因此它们不可相加、不可当作总耗时的百分比、也不可反演成串行占比。
