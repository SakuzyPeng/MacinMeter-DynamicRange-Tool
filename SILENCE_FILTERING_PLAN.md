...
- **实现参数**（已支持）：
  - `--trim-edges[=<DB>]`：启用边缘裁切，可指定静音阈值（默认 -60 dBFS，范围 -120…0 dB）
  - `--trim-min-run`：最小连续静音持续时长（默认 60ms，范围 50–2000ms）
...
- 输出：DR 结果 TXT 会附带阈值与各声道窗口统计（过滤数/总数/百分比），方便对比实验
- 复杂度：时间 O(N) 单遍、空间 O(min_run_frames × channels) 环形缓冲
- 验收状态：
  - ✅ 在 formatTEST 中测试确认：短首尾静音正确回灌，长静音（>0.8s）按默认参数裁掉
  - ✅ test_compatibility.aac：裁切约 0.84s 头部 + 4.5s 尾部后，Precise DR 与 WAV 基准差异 ≤0.02 dB（10.27 vs 10.24）
...
- 风险：把极弱音乐段（如古典）误判为静音。
  - 缓解：保守阈值（默认 −60 dBFS，可按需放宽到 −65…−70）、最小持续时长、迟滞，并仅裁首尾。
- 风险：容器元数据缺失/错误。
